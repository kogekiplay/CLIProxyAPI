...
func BenchmarkAllowedAuthIDCacheForAPIKeyRestricted1000(b *testing.B) {
	manager, _, _ := benchmarkManagerSetup(b, 1000, false, false)
	manager.SetConfig(&internalconfig.Config{
		SDKConfig: internalconfig.SDKConfig{
			APIKeyAccess: map[string]internalconfig.APIKeyAccessRule{
	...
	if ids, cacheKey, restricted := manager.AllowedAuthIDCacheForAPIKey("bench-key"); !restricted || len(ids) != 1000 || cacheKey == "" {
		b.Fatalf("warmup AllowedAuthIDCacheForAPIKey restricted=%v ids=%d cacheKey=%q", restricted, len(ids), cacheKey)
	}

	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ids, cacheKey, restricted := manager.AllowedAuthIDCacheForAPIKey("bench-key")
		if !restricted || len(ids) != 1000 || cacheKey == "" {
			b.Fatalf("AllowedAuthIDCacheForAPIKey restricted=%v ids=%d cacheKey=%q", restricted, len(ids), cacheKey)
		}
	}
}
