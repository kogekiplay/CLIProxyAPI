...
func (e *CodexWebsocketsExecutor) readUpstreamLoop(sess *codexWebsocketSession, conn *websocket.Conn) {
	...
	for {
		...
		if errRead != nil {
			...
			if ch != nil {
				_ = sendTerminalWebsocketRead(ch, done, codexWebsocketRead{conn: conn, err: errRead}, nil)
		...
		if msgType != websocket.TextMessage {
			if msgType == websocket.BinaryMessage {
				...
				if ch != nil {
					_ = sendTerminalWebsocketRead(ch, done, codexWebsocketRead{conn: conn, err: errBinary}, nil)
					if sess.clearActive(conn, ch) {
						close(ch)
					}
				}
